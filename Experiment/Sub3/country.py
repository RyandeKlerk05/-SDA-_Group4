...
df_mh = pd.read_csv(BASE_DIR / "Data/merged_final.csv")
df_users = pd.read_csv(BASE_DIR / "Data/social_media_users_2021.csv")
df_pop = pd.read_csv(BASE_DIR / "Data/IHME-GBD_2023_DATA-aeeaa03f-1.csv")
...
df["SocialMediaShare"] = (df["Social Media Users (N)"] /
                          df["Population"]) * 100
...
plt.plot(x_line, y_line, color="#DD8452", linewidth=2.5,
         label="Regression Line")
plt.xlabel("Social Media Share (% of Population)", fontsize=12)
plt.title("Linear Regression: Social Media Share vs Mental Health",
...
plt.xlabel("Social Media Share (% of Population)", fontsize=12)
plt.ylabel("Mental Health Prevalence (%)", fontsize=12)
plt.title("K-Means Clustering: Social Media vs Mental Health",
          fontsize=14, fontweight="bold")
plt.grid(alpha=0.3)
plt.legend()
plt.tight_layout()
...
df_cont = pd.DataFrame(list(continent_map.items()),
...
plt.title("Average Social Media Share by Continent",
          fontsize=14, fontweight="bold")
